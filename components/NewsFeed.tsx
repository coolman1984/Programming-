    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
      {news.map((item) => (
        <div
          ...
          className="bg-[#0f172a] border border-slate-800 rounded-3xl p-8 relative overflow-hidden h-full flex flex-col shadow-lg hover:border-amber-500/30 hover:shadow-amber-500/10 transition-all cursor-pointer group"
        >
          {/* Static Decoration */}
          ...
          <div className="flex justify-between items-start mb-6 relative z-10">
            ...
            <div className="flex items-center gap-2 px-3 py-1.5 bg-slate-900 rounded-lg border border-slate-800">
              ...
              <span className={`text-sm font-bold ${item.sentiment === 'positive' ? 'text-emerald-500' :
          ...
          {/* Title */}
          <h4 className="text-white font-bold text-lg mb-4 leading-normal font-serif relative z-10 border-b border-slate-800/50 pb-4 text-left">
            {item.title}
          </h4>

          {/* Rich Content */}
          {/* Rich Content - Expanded for Professional Summary */}
          <p className="text-slate-300 text-base leading-relaxed font-medium text-left relative z-10 flex-grow line-clamp-4 mb-6">
            {item.summary}
          </p>

          {/* Footer Time & Action */}
          <div className="mt-auto pt-4 border-t border-slate-800/50 flex justify-between items-center relative z-10">
            <span className="text-xs text-amber-500 font-bold opacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-1">
